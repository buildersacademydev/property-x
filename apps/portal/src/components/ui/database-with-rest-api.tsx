...
const DatabaseWithRestApi = ({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto flex h-auto w-full max-w-4xl flex-col items-center",
        className
      )}
    >
      {/* SVG Paths  */}
      <svg
        className="h-full text-primary/30 sm:w-full"
        width="100%"
        height="100%"
        viewBox="0 0 200 100"
      >
        ...
        {/* Buttons */}
        <g stroke="hsl(var(--border))" fill="none" strokeWidth="0.5">
          {/* First Button */}
          <g>
            <rect
              fill="hsl(var(--card))"
        ...
        <defs>
          ...
          <radialGradient id="db-blue-grad" fx="1">
            <stop offset="0%" stopColor={lightColor || "hsl(var(--primary))"} />
      ...
      <div className="absolute bottom-10 flex w-full flex-col items-center">
        ...
        <div
          className="absolute -top-3 z-20 flex items-center justify-center
            rounded-lg border bg-card px-2 py-1 sm:-top-4 sm:py-1.5"
        >
        ...
        {/* box outter circle */}
        <div
          className="absolute -bottom-8 z-30 grid h-[60px] w-[60px]
            place-items-center rounded-full border-t bg-card text-xs
            font-semibold"
        >
        ...
        {/* box content */}
        <div
          className="relative z-10 flex h-[150px] w-full items-center
            justify-center overflow-hidden rounded-lg border bg-background
            shadow-md"
        >
          {/* Badges */}
          <div
            className="absolute bottom-8 left-12 z-10 flex h-7 items-center
              gap-2 rounded-full border bg-card px-3 text-xs"
          >
            <HeartHandshakeIcon className="size-4" />
            <span>{buttonTexts?.first || "LegionDev"}</span>
          </div>
          <div
            className="absolute right-16 z-10 hidden h-7 items-center gap-2
              rounded-full border bg-card px-3 text-xs sm:flex"
          >
